...
import SystemDemo from './scripts/systemdemo.js'
...
function OnAddinLoad(ribbonUI) {
    ...
    // 启动后1秒后打开任务窗格
    setTimeout(() => {
        ToggleTaskPane(`${Util.GetUrlPath()}${Util.GetRouterHash()}`)
    }, 1000)
...
function OnAction(control) {
    ...
    switch (eleId) {
        case "btnShowMsg":
            {
                ...
                if (!doc) {
                    Util.wpsAlert("当前没有打开任何文档")
                    return
                }
                Util.wpsAlert(doc.Name)
            }
            break;
        case "btnIsEnbable":
            {
        ...
        case "btnShowDialog":
            window.Application.ShowDialog(Util.GetUrlPath() + Util.GetRouterHash() + "/dialog", "这是一个对话框网页", 600 * window.devicePixelRatio, 600 * window.devicePixelRatio, false)
            break
        case "btnShowTaskPane":
            {
                ToggleTaskPane(`${Util.GetUrlPath()}${Util.GetRouterHash()}/taskpane`)
            }
            break
        case "btnApiEvent":
            {
...
function OnNewDocumentApiEvent(doc) {
    Util.wpsAlert("新建文件事件响应，取文件名: " + doc.Name)
}

function ToggleTaskPane(url) {
